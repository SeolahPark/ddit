...
public class FileController extends HttpServlet {
	...
	IFilesService fileService = FilesService.getInstance();
...
//						break;
		...
		response.sendRedirect(request.getContextPath()+forward); //무조건 get
	}
	...
	private void insertFiles(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, SQLException {
		...
		int file_category = Integer.parseInt(request.getParameter("file_category"));
		String path = "profile";
		if(file_category==1) path = "board";
		String uploadPath = "C:/Users/seolah/Documents/Java/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/MYBOARD/img/"+path;
...
//		for(Part part : request.getParts()) {
		...
		if(part != null) {
			...
			if(!"".equals(fileName)) {
				...
				case 1:
					map.put("board_no", request.getParameter("board_no"));
					forward = "/board/List.do?page=1";
					break;
				case 2:
					map.put("board_no", null);
					forward = "/member/myPage.do";
				case 3:
//					map.put("col", "SERVICE_NO");
//					map.put("no", request.getParameter("service_no"));
...
//					break;
	...
	// 파일 업로드 하는 메소드
	public static MultipartRequest insertFiles(HttpServletRequest request, String saveDirectory, int maxPostSize) {
		try {
	...
	private void imageView(HttpServletRequest request, HttpServletResponse response) throws SQLException, UnsupportedEncodingException {
		...
		map.put("file_category_no", file_category);
...
//		String filepath = request.getRealPath("/upload/" + fileService.selectFileCategoty(num));
		String filepath = fileService.selectFileCategoty(file_category);
